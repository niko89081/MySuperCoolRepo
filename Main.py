...
blurerererer = cv.blur(gray, (3, 3))

circles = cv.HoughCircles(blurerererer, cv.HOUGH_GRADIENT, 1, 100101, minRadius=230, maxRadius=250)

if circles is not None:
    circles = np.round(circles[0, :]).astype("int")
    for (x, y, r) in circles:
        cv.circle(result, (x, y), r, (0, 255, 0), 7)
